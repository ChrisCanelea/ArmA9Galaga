    ...
    while (!gameStart) {
        ...
        if (PS2data & 0x8000) { // check RVALID
            ...
            if ((byte1 == 0xF0) || (byte2 == 0xF0) || (byte3 == 0xF0)) {
                ...
                while (PS2data & 0x8000) { //clear remaining input
                    byte1 = byte2;
                    byte2 = byte3;
                    byte3 = PS2data & 0xFF;
                }
...
int gameLoop() {
    ...
    *(PS2) = 0xFF; // reset keyboard
    int inputStatus[3] = {0, 0, 0}; // L R Shooting
    ...
    {   
        ...
        if (((inputStatus[0] == 0) && (inputStatus[1] == 0)) || ((inputStatus[0] == 1) && (inputStatus[1] == 1))){
            player.hitbox.dx = 0; //no movement on arrow key release
        } else if (inputStatus[1] == 1) {
            player.hitbox.dx = 2;
        } else if (inputStatus[0] == 1) {
            player.hitbox.dx = -2;
        }
...
// reads one input per call
void readPS2(int inputStatus[]) {
    int PS2data;
    char byte1 = 0, byte2 = 0, byte3 = 0;
    for (int i = 0; i < 3; i++) {
        PS2data = *(PS2); // read ps2 data
        if (PS2data & 0x8000) { // check RVALID
            byte1 = byte2;
            byte2 = byte3;
            byte3 = PS2data & 0xFF;
    ...
    if ((byte2 == 0xF0) && (byte3 == 0x6B)) {
        inputStatus[0] = 0;
    } else if ((byte2 == 0xF0) && (byte3 == 0x74)) {
        inputStatus[1] = 0;
    } else if ((byte2 == 0xF0) && (byte3 == 0x29)) {
        inputStatus[2] = 0;
    } else if (byte3 == 0x6B) {
        inputStatus[0] = 1;
    } else if (byte3 == 0x74) {
